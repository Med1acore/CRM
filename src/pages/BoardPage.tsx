    ...
    cards: [
      {
        id: 'card-1',
        list_id: 'list-1',
        title: 'Подготовить графику для воскресного служения',
        position: 0,
      },
...
export function BoardPage() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    const activeCard = lists.flatMap((list) => list.cards).find((card) => card.id === activeId);
        ...
        prevLists.map((list) => {
          if (list.id === activeCard.list_id) {
            ...
            return {
              ...
              cards: [
                ...list.cards,
                { ...activeCard, list_id: targetListId, position: list.cards.length },
              ],
            };
          }
          return list;
    ...
    <div className="p-8 h-full flex flex-col bg-gray-50 dark:bg-gray-900">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">{boardName}</h1>
            <p className="text-gray-600 dark:text-gray-400">
              Доска задач • {lists.length} колонок •{' '}
              {lists.reduce((acc, list) => acc + list.cards.length, 0)} задач
            </p>
          </div>
        </div>
        ...
        <DragOverlay>
          {activeCard ? (
            <div className="p-3 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg shadow-lg opacity-90 rotate-3">
              <p className="text-sm text-gray-900 dark:text-gray-100">{activeCard.title}</p>
            </div>
          ) : null}
        </DragOverlay>
      </DndContext>
